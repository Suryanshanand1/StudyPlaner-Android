...
import { getToday, getWeekDays, formatDate, formatTime } from "@/lib/utils"
import PlanForm from "./PlanForm"

export default function StudyPlanner() {
  const { studyPlans, subjects, deleteStudyPlan, getWeekPlans } = useStore()
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            {dayPlans.map((plan) => {
              ...
              return (
                <div key={plan.id} className="flex items-center justify-between rounded-xl border border-zinc-100 bg-white p-3 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                  <div className="flex items-center gap-3">
                    <div className="h-3 w-3 rounded-full" style={{ backgroundColor: sub?.color }} />
                    <div>
                      <p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{plan.subjectName} - {plan.chapterName}</p>
                  ...
                  return (
                    <div key={plan.id} className="flex items-center justify-between py-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-zinc-700 dark:text-zinc-300">{plan.chapterName}</span>
